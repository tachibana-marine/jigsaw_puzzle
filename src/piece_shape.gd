...
func _update_polygon():
  ...
  var center_x = (
    _get_bottom_right_of_polygon(dimple_shape).x - _get_bottom_left_of_polygon(dimple_shape).x
  )
  center_x /= 2
  for coord in coords:
    var d = dimple[coord]
  ...
  if is_cavity[3] != null:
    vertices += (_get_dimple_shape(
      size.x, size.y - (positions[3] - center_x), 0.5 * PI, is_cavity[3]
    ))
  queue_redraw()
...
func _draw() -> void:
  ...
  #   if i == 0:
  #     continue
  #   draw_line(vertices[i - 1], vertices[i], Color.WHITE, 2)
